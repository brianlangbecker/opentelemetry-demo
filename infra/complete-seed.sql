-- Add orderitems for all orders without items
...
-- Add shipping for all orders without shipping
...
SELECT 
    ...
    (SELECT COUNT(*) FROM orderitem) as items,
    (SELECT COUNT(*) FROM shipping) as shipments,
    pg_size_pretty(pg_database_size('otel')) as db_size;
